...
get_mvb_term <- function(label) {
  ...
  label_clean <- gsub("^behavior according to$", "behaviour according to", label_clean)
  label_clean <- gsub("^study timezone$", "study time zone", label_clean)
  label_clean <- gsub("^study local timestamp$", "local timestamp", label_clean)
