...
- Initiate a package with `create_package()`. This and other functions are reexported in movepub from the [frictionless](https://docs.ropensci.org/frictionless/) R package.
...
`write_dwc()` requires the dataset to be structured as a [Data Package](https://datapackage.org/) (recognizable by the presence of a `datapackage.json` file). That is the case for O_ASSEN on Zenodo. See the previous section to create your own Data Package.
...
That covers the data. The Darwin Core transformation also needs some metadata for record-level terms (e.g. `dwc:datasetName`, `dcterms:license`, etc.). By default, these are derived from the package metadata (i.e. [Data Package properties](https://datapackage.org/standard/data-package)). O_ASSEN for example, has the DOI in `package$id`, which will be used for `dwc:datasetID`:
...
This time, the dataset does not need to be a Data Package. The only requirement is that it is published and has a DOI:
